...
# Towards the next generation of Shiny UI {background-image="ship.webp" background-size="contain" style='--r-heading-color: white; color: white;'}
...
2.  Provide modern UI replacements for `{shiny}` and `{shinydashboard}`.
    -  <b>Currently includes page and layout helpers, sidebars, cards, value boxes, accordions, and more.</b>
    -  Eventually, it should include more like inputs, tooltips, busy indication, and more.
    -  We plan on integrating this work into the [Shiny UI editor](https://rstudio.github.io/shinyuieditor/) and [Shiny for Python](https://shiny.rstudio.com/py/).
:::
...
::: {.font-sm}
- Easily choose main colors and fonts 
...
::: {.font-sm}
There are 100s of [theming variables](https://rstudio.github.io/bslib/articles/bs5-variables.html) to customize.
:::
...
Much of this is still in development, but if you're brave, try it out:
...
::: {.incremental}
-  Steeper learning curve than `{shinydashboard}` & `{flexdashboard}`, but you'll gain flexibility, portability, theme-ability, etc.
- You may miss some `{shinydashboard}` features
    - Like nav inside a sidebar, but IMO this is an anti-pattern (keep nav in the header and/or card(s)).
    - As always, we love to hear your feedback, and ideas for new components!
-  Not everything needs to be dashboard (and/or use Shiny)! We hope these components are generically useful and portable (in R Markdown, static sites, eventually Quarto, etc.)
:::
...
-  Learn more at [rstudio.github.io/bslib](https://rstudio.github.io/bslib)
    - Big doc updates are coming soon.
