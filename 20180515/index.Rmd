...
## however, a web-based platform introduces a whole new set of concerns...
...
TODO: a ggplot2 slider example?


---

Another example, accessing url pathname

Note how `input()` takes a component *id* and *property*
...
Another example, demonstrating `state()` (like `input()` but doesn't trigger a callback).
...
app <- Dash$new()
app$layout_set(
  coreInput(id = "a", type = "text"),
  htmlButton(id = "b", "submit"),
  htmlDiv(id = "c")
)
app$callback(
  function(
    value = state("a", "value"),
    n_clicks = input("b", "n_clicks")
  ) {
    sprintf(
...
Note how `input()`, `state()`, and `output()` need a component *id* and *property* (`input()`/`state()` default to `"value"`, `output()` defaults to `"children"`) 

---
class: principles

One exception: [htmlwidgets for R](http://www.htmlwidgets.org/) in dashR!
