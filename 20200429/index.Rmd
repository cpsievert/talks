...
## What about when auto-theming fails?

* In some scenarios (e.g., non-HTML output formats in **rmarkdown**, R terminal, etc), auto-theming "fails"
  * That is, it defaults to the usual white background + black foreground.
  
* There are two ways to resolve auto-theming failure:
  * Set defaults via `auto_defaults_set()`
  * Specify colors (and/or fonts) directly in `thematic_on()`
  
* Providing non-auto values to `thematic_on()`/`auto_defaults_set()` is also useful for customizing theming defaults 
...
* Provide values directly to `thematic_on()` and/or `auto_defaults_set()` to customize.
