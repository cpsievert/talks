...
Any **shiny** output with a special `.shiny-report-theme` class will soon report it's computed CSS styles.
...
The new [**bootstraplib** package](https://rstudio.github.io/bootstraplib/index.html) makes it possible to style **shiny** and **rmarkdown** in R (via **sass**) instead of CSS.

Check out [this article](https://rstudio.github.io/bootstraplib/articles/recipes.html) for a good sense of where things are (more to come).

(BTW, **bootstraplib** is what makes auto-theming in `html_document()` feasible) 
