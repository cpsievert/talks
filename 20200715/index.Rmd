...
.panel[.panel-name[Code]
```{r flatly2, eval = FALSE, echo = FALSE}
library(shiny)
library(bootstraplib)
shinyOptions(bootstraplib = TRUE)
...
```{r, echo = FALSE}
decorate("flatly2", eval = FALSE) %>%
  flair('bootswatch = "flatly"') 
```
...
* __thematic__'s [auto-theming](https://rstudio.github.io/thematic/articles/auto.html) essentially translates CSS styles into new R defaults. It works:
  ...
  * With RStudio and R Markdown (if styling is done via **bootstraplib**)
]
]
...
### Can also detect and render custom fonts! 🎉 
...
### Styling R Markdown's `html_document()`
